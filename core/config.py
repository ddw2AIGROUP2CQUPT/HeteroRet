...
# 在 core/config.py 文件中

# ... (其他配置项) ...

# NEW: Add a new block for inference/extraction settings
_C.INFER = CN()

# Path to the trained model weights
_C.INFER.WEIGHTS = ""

# Path to the input image(s) or directory
_C.INFER.INPUT_PATH = ""

# Path to save the output features
_C.INFER.OUTPUT_FILE = ""
_C.INFER.BATCH_SIZE = 32      # 新增
_C.INFER.IM_SIZE = 256        # 新增
_C.INFER.NUM_WORKERS = 32    # 新增
_C.INFER.TOTAL_NUM = 8       # 新增
_C.INFER.SCALE_LIST=[1.0]
# ... (文件其余部分) ...
...
# ViT模型名称 (timm支持的模型名)
_C.VIT.MODEL_NAME = "vit_base_patch16_224"

# 是否使用预训练权重
_C.VIT.PRETRAINED = True
...
# 嵌入维度 (自动从timm模型获取，这里仅作记录)
_C.VIT.EMBED_DIM = 768

# Transformer层数 (自动从timm模型获取，这里仅作记录)
_C.VIT.NUM_LAYERS = 12

# 注意力头数 (自动从timm模型获取，这里仅作记录)
_C.VIT.NUM_HEADS = 12

# MLP隐藏层倍数 (自动从timm模型获取，这里仅作记录)
_C.VIT.MLP_RATIO = 4.0

# Dropout率 (自动从timm模型获取，这里仅作记录)
_C.VIT.DROPOUT = 0.0
...
# "vit_base_patch32_224"     - ViT-Base with 32x32 patches
# "deit_base_patch16_224"    - DeiT-Base
# "swin_base_patch4_window7_224" - Swin Transformer

# 在assert_and_infer_cfg函数中添加ViT相关验证
def assert_and_infer_cfg(cache_urls=True):
    ...
    # 添加ViT相关验证
    if _C.MODEL.TYPE == "vit":
        err_str = "ViT model name not specified"
        assert _C.VIT.MODEL_NAME != "", err_str
        
        # 验证图像尺寸
        if "224" in _C.VIT.MODEL_NAME:
            recommended_size = 224
        elif "384" in _C.VIT.MODEL_NAME:
            recommended_size = 384
        else:
            recommended_size = 224
            
        if _C.TRAIN.IM_SIZE != recommended_size:
            print(f"Warning: Train image size {_C.TRAIN.IM_SIZE} may not match ViT model {_C.VIT.MODEL_NAME} (recommended: {recommended_size})")
        if _C.TEST.IM_SIZE != recommended_size + 32:  # 测试时通常用更大尺寸
            print(f"Warning: Test image size {_C.TEST.IM_SIZE} may not be optimal for ViT model {_C.VIT.MODEL_NAME}")
